...
export class ExportManager {
    logger;
    database;
    constructor(database, _config, logger) {
     ...
     */
    async exportToJSON(entries, options) {
     ...
     */
    async exportToCSV(entries, options) {
        const data = this.prepareDataForExport(entries, options);
        ...
        const rows = data.map(entry => {
            ...
            if (options.includeMetrics && entry.metrics) {
                row.push(entry.metrics.memoryUsage.toString(), entry.metrics.cpuUsage.toString(), entry.metrics.rulesApplied.toString(), entry.metrics.cacheHitRatio.toString());
            }
            return row.join(',');
        });
     ...
     */
    async exportToXLSX(entries, options) {
        try {
     ...
     */
    async exportToPDF(entries, options) {
        try {
    ...
    async generateExportFile(entries, options) {
    ...
    prepareDataForExport(entries, options) {
        return entries.map(entry => {
            const exportEntry = { ...entry };
            if (!options.includeSourceCode) {
                exportEntry.sourceCode = undefined;
            }
            if (!options.includeMetrics) {
                exportEntry.metrics = undefined;
            }
            return exportEntry;
        });
    }
    ...
    async ensureExportDirectory(options) {
