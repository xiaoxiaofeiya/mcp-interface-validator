...
export class RetryManager {
     ...
     */
    async execute(operation, config, context) {
        let lastError;
    ...
    _categorizeError(error) {
        ...
        if (this._isNetworkError(message))
            return ErrorCategory.NETWORK;
        if (this._isTimeoutError(message))
            return ErrorCategory.TIMEOUT;
        if (this._isRateLimitError(message))
            return ErrorCategory.RATE_LIMIT;
        if (this._isResourceError(message))
            return ErrorCategory.RESOURCE;
        if (this._isSystemError(message))
            return ErrorCategory.SYSTEM;
        if (this._isValidationError(message))
            return ErrorCategory.VALIDATION;
        if (this._isAuthenticationError(message))
            return ErrorCategory.AUTHENTICATION;
        if (this._isAuthorizationError(message))
            return ErrorCategory.AUTHORIZATION;
        if (this._isConfigurationError(message))
            return ErrorCategory.CONFIGURATION;
        return ErrorCategory.UNKNOWN;
    }
    _isNetworkError(message) {
        return ['econnrefused', 'enotfound', 'econnreset', 'etimedout', 'network', 'connection'].some(keyword => message.includes(keyword));
    }
    _isTimeoutError(message) {
    ...
    _isResourceError(message) {
        return ['not found', '404', 'resource', 'file not found'].some(keyword => message.includes(keyword));
    }
    _isSystemError(message) {
        return ['internal server error', '500', 'system', 'server', 'service unavailable', '503'].some(keyword => message.includes(keyword));
    }
    _isValidationError(message) {
