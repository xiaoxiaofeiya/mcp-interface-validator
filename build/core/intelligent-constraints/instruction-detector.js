...
export class InstructionDetector {
    ...
    initializeCommandPatterns() {
        ...
        this.commandPatterns.set('.use interface', /^\.use\s+interface(?:\s+(.+))?$/i);
        // .使用接口 模式 - 允许可选的用户指令
        this.commandPatterns.set('.使用接口', /^\.使用接口(?:\s+(.+))?$/);
        // .apply constraints 模式 - 允许可选的用户指令
        this.commandPatterns.set('.apply constraints', /^\.apply\s+constraints(?:\s+(.+))?$/i);
        // .应用约束 模式 - 允许可选的用户指令
        this.commandPatterns.set('.应用约束', /^\.应用约束(?:\s+(.+))?$/);
        // .interface mode 模式 - 允许可选的用户指令
        this.commandPatterns.set('.interface mode', /^\.interface\s+mode(?:\s+(.+))?$/i);
        // .接口模式 模式 - 允许可选的用户指令
        this.commandPatterns.set('.接口模式', /^\.接口模式(?:\s+(.+))?$/);
        // 更灵活的模式匹配 - 允许可选的用户指令
        this.commandPatterns.set('.use interface (flexible)', /^\.use\s+interface\s*[:\-]?\s*(.*)$/i);
        this.commandPatterns.set('.使用接口 (flexible)', /^\.使用接口\s*[：\-]?\s*(.*)$/);
     ...
     * 从匹配结果中提取指令
     */
    extractInstructionFromMatch(input, match) {
        // 获取捕获组中的用户指令
     ...
     * 计算置信度
     */
    calculateConfidence(_input, command) {
     ...
     * 添加自定义指令模式
     */
    addCustomCommand(command, pattern) {
