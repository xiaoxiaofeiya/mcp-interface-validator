...
export class HtmlReportGenerator extends EventEmitter {
     ...
     */
    async generateValidationSummary(validationResults, options = {}) {
        this.ensureInitialized();
        const reportData = {
            metadata: {
     ...
     */
    async exportReport(htmlContent, format, outputPath) {
        this.ensureInitialized();
        try {
            switch (format) {
                case 'html':
     ...
     * Add custom template
     */
    async addCustomTemplate(name, templateContent) {
        try {
...
<body>
    ...
    registerHelpers() {
        ...
        Handlebars.registerHelper('statusBadge', (status) => {
            ...
            return new Handlebars.SafeString(`<span class="badge badge-${status}" style="background-color: ${color}; color: white; padding: 2px 8px; border-radius: 4px;">${status}</span>`);
        });
        // Chart rendering helper
        Handlebars.registerHelper('renderCharts', (charts) => {
    ...
    prepareTemplateContext(data, options) {
        return {
            ...data,
            options,
            timestamp: new Date(),
    ...
    async saveReport(content, outputPath, _format) {
    ...
    generateValidationCharts(validationResults) {
        const charts = [];
        ...
        charts.push({
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: { display: true, text: 'Validation Status Distribution', position: 'top' }
                }
            }
        });
        ...
        charts.push({
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: { display: true, text: 'Error Distribution by Specification', position: 'top' }
                },
                scales: {
                    y: {
                        type: 'linear',
        ...
        if (responseTimes.length > 0) {
            charts.push({
                ...
                options: {
                    ...
                    maintainAspectRatio: false,
                    plugins: {
                        title: { display: true, text: 'Response Times Over Time', position: 'top' }
                    },
                    scales: {
                        y: {
                            type: 'linear',
        ...
        if (throughputMetrics.length > 0) {
            charts.push({
                ...
                options: {
                    ...
                    maintainAspectRatio: false,
                    plugins: {
                        title: { display: true, text: 'Throughput Metrics', position: 'top' }
                    },
                    scales: {
                        y: {
                            type: 'linear',
    ...
    generateOverviewContent(validationResults) {
    ...
    generateDetailsContent(validationResults) {
    ...
    generatePerformanceOverview(metrics) {
    ...
    async exportAsHtml(content, outputPath) {
        await fs.writeFile(outputPath, content, 'utf-8');
    }
    async exportAsPdf(htmlContent, outputPath) {
...
Note: Full PDF generation requires additional dependencies like puppeteer or similar.`;
    ...
    async exportAsJson(htmlContent, outputPath) {
        const jsonData = {
