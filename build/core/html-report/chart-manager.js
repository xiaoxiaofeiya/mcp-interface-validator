...
export class ChartManager {
    _logger; // Used for logging chart operations
     ...
     */
    generateValidationStatusChart(validationResults) {
     ...
     */
    generateErrorDistributionChart(validationResults) {
     ...
     */
    generateCoverageChart(validationResults) {
     ...
     */
    generatePerformanceTimelineChart(metrics) {
     ...
     */
    generateResponseTimeHistogram(metrics) {
     ...
     */
    generateThroughputChart(metrics) {
        ...
        return {
            ...
            data: {
                ...
                datasets: [{
                        label: 'Requests/sec',
                        data: Object.values(groupedData).map(values => values.reduce((sum, val) => sum + val, 0) / values.length),
     ...
     */
    generateCustomChart(type, title, data, customOptions) {
    ...
    aggregateValidationStatus(validationResults) {
    ...
    calculateAverageCoverage(validationResults) {
    ...
    prepareTimelineData(metrics) {
        // Group metrics by category
        const categories = [...new Set(metrics.map(m => m.category))];
        const sortedMetrics = metrics.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
        // Create time labels
        const labels = [...new Set(sortedMetrics.map(m => m.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })))];
        // Create datasets for each category
        const datasets = categories.map((category, index) => {
            const categoryMetrics = sortedMetrics.filter(m => m.category === category);
            const data = labels.map(label => {
                const metric = categoryMetrics.find(m => m.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) === label);
    ...
    getDefaultBarOptions(title, yAxisLabel) {
        return {
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
    ...
    getDefaultLineOptions(title, xAxisLabel, yAxisLabel) {
        return {
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
