...
class MCPInterfaceValidator {
     ...
     */
    async handleValidateInterface(args) {
        try {
            const { interfaceCode, specPath, validationType = 'both', userInstruction = '', projectContext = {}, sessionId = '', enableConstraints = false } = args;
            ...
            if (enableConstraints && sessionId && this.constraintSystem.getConstraintStatus(sessionId).isActive) {
                ...
                if (userInstruction) {
                    const constraintResult = await this.constraintSystem.applyConstraintsToInstruction(userInstruction, sessionId, projectContext);
                    if (constraintResult.success && constraintResult.enhancedInstruction) {
                        ...
                        const result = await this.validationEngine.validateWithIntelligentContext(enhancedInstruction, interfaceCode, specPath, projectContext);
            ...
            if (userInstruction && interfaceCode) {
                const result = await this.validationEngine.validateWithIntelligentContext(userInstruction, interfaceCode, specPath, projectContext);
                return {
                    content: [
                        {
                            type: 'text',
     ...
     */
    formatIntelligentValidationResult(result) {
        let output = '';
        // Header
        output += `🧠 智能接口验证结果\n`;
        output += `==================\n\n`;
        ...
        if (result.contextSuggestions) {
            ...
            if (suggestions.suggestedEndpoints.length > 0) {
                output += `🎯 建议的API端点\n`;
                suggestions.suggestedEndpoints.forEach((endpoint) => {
                    output += `- ${endpoint}\n`;
                });
                output += `\n`;
            }
     ...
     */
    async handleMonitorChanges(args) {
        try {
     ...
     */
    async handleActivateConstraints(args) {
        try {
            ...
            const result = await this.constraintSystem.processUserInput(userInput, sessionId, projectContext);
            if (result.error) {
     ...
     */
    async handleApplyConstraints(args) {
        try {
            const { userInstruction, sessionId, constraintType = 'default', projectContext = {} } = args;
            ...
            const result = await this.constraintSystem.applyConstraintsToInstruction(userInstruction, sessionId, projectContext);
            if (!result.success) {
