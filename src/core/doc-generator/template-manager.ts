...
export class TemplateManager {
   ...
   */
  async loadTemplate(config: TemplateConfig): Promise<HandlebarsTemplateDelegate> {
    try {
   ...
   */
  private async loadCustomTemplate(templateDir: string): Promise<string> {
   ...
   */
  private async loadBuiltinTemplate(templateName: string): Promise<string> {
...
*Generated on {{formatDate metadata.generatedAt}} by API Documentation Generator v{{metadata.generatorVersion}}*`;
  ...
  private createHelpers(): TemplateHelpers {
    return {
      ...
      anchor: (text: string) => text.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
      httpMethod: (method: string) => method.toUpperCase(),
      statusCode: (code: string) => code,
      hasItems: (arr: any[]) => Array.isArray(arr) && arr.length > 0,
   ...
   */
  private getTemplateKey(config: TemplateConfig): string {
   ...
   */
  private createTemplateError(code: string, message: string, cause?: any): DocumentationError {
