...
export class CodeExampleGenerator {
   ...
   */
  private async generateRequestExample(
    operation: OperationData,
    language: string,
    config: CodeExampleConfig
   ...
   */
  private generateTypeScriptRequest(operation: OperationData, _config: CodeExampleConfig): string {
   ...
   */
  private generateJavaScriptRequest(operation: OperationData, config: CodeExampleConfig): string {
   ...
   */
  private generatePythonRequest(operation: OperationData, _config: CodeExampleConfig): string {
    const { method, path, parameters, requestBody } = operation;
   ...
   */
  private generateJavaRequest(operation: OperationData, _config: CodeExampleConfig): string {
   ...
   */
  private generateCurlRequest(operation: OperationData, _config: CodeExampleConfig): string {
    const { method, path, parameters, requestBody } = operation;
   ...
   */
  private async generateResponseExamples(
    operation: OperationData,
    _language: string,
    _config: CodeExampleConfig
  ): Promise<CodeExample[]> {
    const examples: CodeExample[] = [];
   ...
   */
  private async generateErrorExamples(
    operation: OperationData,
    _language: string,
    _config: CodeExampleConfig
  ): Promise<CodeExample[]> {
    ...
    const errorResponses = operation.responses.filter(r => 
   ...
   */
  private generateResponseJson(response: any): string {
   ...
   */
  private createExampleError(code: string, message: string, cause?: any): DocumentationError {
    const error = new Error(message) as DocumentationError;
    error.code = code;
    error.context = { cause };
    error.suggestions = [
