 ...
 */
export class SQLiteDatabase extends EventEmitter implements IHistoryDatabase {
  private db: any = null;
  ...
  protected async openDatabase(sqlite3: any, dbPath: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      const dbInstance = new sqlite3.Database(dbPath, (err: Error) => {
        if (err) {
          reject(err);
        } else {
          resolve(dbInstance);
  ...
  private async beginTransaction(): Promise<ITransaction> {
    ...
    return {
      ...
      store: async (entry: ValidationHistoryEntry) => {
  ...
  private buildQuery(query: HistoryQuery): { sql: string; params: any[]; countSql: string } {
    let whereClause = '';
    let params: any[] = [];
  ...
  private buildWhereClause(filters: { specId?: string; dateRange?: { start: Date; end: Date } }): string {
    const conditions: string[] = [];
  ...
  private buildWhereParams(filters: { specId?: string; dateRange?: { start: Date; end: Date } }): any[] {
    const params: any[] = [];
  ...
  private mapRowToEntry(row: any): ValidationHistoryEntry {
    return {
      id: row.id,
      specId: row.spec_id,
