...
export class CleanupManager extends EventEmitter {
   ...
   */
  start(scheduleConfig?: { schedule: string; retentionDays: number }): void {
    if (this.isRunning()) {
   ...
   */
  async dryRun(options?: Partial<CleanupOptions>): Promise<CleanupResult> {
   ...
   */
  async cleanupOlderThan(days: number, options?: Partial<CleanupOptions>): Promise<CleanupResult> {
   ...
   */
  async cleanupFailedOnly(days: number, options?: Partial<CleanupOptions>): Promise<CleanupResult> {
   ...
   */
  async cleanupSuccessfulOnly(days: number, options?: Partial<CleanupOptions>): Promise<CleanupResult> {
    return await this.cleanup({
      ...options,
      retentionDays: days,
   ...
   */
  async estimateCleanup(options: Partial<CleanupOptions>): Promise<CleanupEstimate>;
  async estimateCleanup(retentionDays: number): Promise<CleanupEstimate>;
  async estimateCleanup(optionsOrRetentionDays: Partial<CleanupOptions> | number): Promise<CleanupEstimate> {
    const options: CleanupOptions = typeof optionsOrRetentionDays === 'number'
      ? {
  ...
  private buildCleanupOptions(options?: Partial<CleanupOptions>): CleanupOptions {
