 ...
 * 约束模板引擎实现
 */
export class ConstraintTemplateEngine implements IConstraintTemplateEngine {
...
⚡ CONFIRM understanding and acceptance of ALL constraints before coding.`,
   ...
   * 选择合适的模板
   */
  private selectTemplate(config: ConstraintConfig): ConstraintTemplate | null {
   ...
   * 生成命名约定
   */
  private generateNamingConvention(userIntent: UserIntent): string {
   ...
   * 格式化响应格式
   */
  private formatResponseFormat(responseFormat: any): string {
    if (!responseFormat) {
   ...
   * 生成自定义规则部分
   */
  private generateCustomRulesSection(customRules: string[], language: 'zh' | 'en'): string {
   ...
   * 验证模板
   */
  private validateTemplate(template: ConstraintTemplate): void {
   ...
   * 检查是否为默认模板
   */
  private isDefaultTemplate(name: string): boolean {
   ...
   * 获取模板描述
   */
  private getTemplateDescription(template: ConstraintTemplate): string {
